...
try:
    ...
    load_dotenv(Path(__file__).resolve().parents[2] / 'backend' / '.env', override=True)
except ImportError:
    pass
...
def _missing_config() -> str | None:
    ...
    if parts.scheme not in {'http', 'https'} or not parts.netloc:
        return 'Supabase URL is invalid. Use the project URL, for example https://PROJECT_ID.supabase.co'
    return None


@st.cache_resource
def get_client() -> Client | None:
    """Cached singleton that preserves PKCE state across Streamlit reruns."""
    if _missing_config():
        return None
